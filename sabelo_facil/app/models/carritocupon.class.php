...
class Detallecupones extends Validator{
    ...
    public function setNumcorrelativo($value){
		if($this->validateNumeric($value, 1, 11)){
			$this->numcorrelativo = $value;
			return true;
		}else{
			return false;
		}
    ...
    public function getVentas(){
		$sql = "SELECT  venta.ID_venta as ventass ,cliente.nombre,cliente.apellido,cliente.correo,venta.fecha, cupones.nombre_cupon, cupones.precio , detalle_factura_cupones.cantidad ,detalle_factura_cupones.sub_total 
        FROM detalle_factura_cupones
        INNER JOIN venta ON detalle_factura_cupones.FK_ID_venta = venta.ID_venta
        INNER JOIN cupones ON detalle_factura_cupones.FK_ID_cupon = cupones.ID_cupon
    ...
    public function getVentas22(){
		$sql = "
        SELECT  venta.ID_venta as ventass ,cliente.nombre,cliente.apellido,cliente.correo,venta.fecha, SUM(detalle_factura_cupones.sub_total) AS totalito ,
        cupones.nombre_cupon, cupones.precio , detalle_factura_cupones.cantidad ,detalle_factura_cupones.sub_total 
        FROM detalle_factura_cupones
        INNER JOIN venta ON detalle_factura_cupones.FK_ID_venta = venta.ID_venta
        INNER JOIN cupones ON detalle_factura_cupones.FK_ID_cupon = cupones.ID_cupon
        INNER JOIN cliente ON venta.FK_ID_cliente = cliente.ID_cliente
    ...
    public function getVentasporfecha($fechainicio,$fechafin){
        ...
        INNER JOIN producto ON detalle_factura.FK_id_cupon = producto.id_cupon
        INNER JOIN cliente ON venta.FK_ID_cliente = cliente.ID_cliente
    ...
    public function searchVenta($value){
        ...
        FROM detalle_factura
        INNER JOIN venta ON detalle_factura.FK_ID_venta = venta.id_venta
        INNER JOIN productos ON detalle_factura.FK_ID_proucto = productos.id_cupon
    ...
    public function viewcarrito(){
        $sql = "SELECT detalle_factura_cupones.ID_detalle,detalle_factura_cupones.FK_ID_cupon,cupones.img_url,cupones.nombre_cupon,detalle_factura_cupones.cantidad,cupones.precio,
        detalle_factura_cupones.sub_total
                FROM detalle_factura_cupones
                INNER JOIN cupones ON detalle_factura_cupones.FK_ID_cupon = cupones.ID_cupon
                WHERE detalle_factura_cupones.FK_ID_cliente = ? AND detalle_factura_cupones.estadoventa = 0";
    ...
    public function viewcarrito2(){
        ...
        detalle_factura_cupones.sub_total
                FROM detalle_factura_cupones
                INNER JOIN cupones ON detalle_factura_cupones.FK_ID_cupon = cupones.ID_cupon
                WHERE detalle_factura_cupones.FK_ID_cliente = ? AND detalle_factura_cupones.estadoventa = 1
        AND detalle_factura_cupones.FK_ID_venta = (SELECT MAX(venta.ID_venta) FROM venta)";
        $params = array($this->id_usuario);
        return Database::getRows($sql,$params);
    }
    ...
    public function facturafinal (){
        $sql="SELECT detalle_factura.ID_detalle ,detalle_factura.FK_id_cupon,producto.imagen_url,producto.nombre_producto
     ...
     public function checkTotal(){
        $sql = "SELECT SUM(sub_total) as totalito
        FROM detalle_factura_cupones
        WHERE detalle_factura_cupones.FK_ID_cliente = ? AND detalle_factura_cupones.estadoventa = 0";
         $params = array($this->id_usuario);
    ...
    public function checkcorrelativo(){
        $sql = "SELECT detalle_factura_cupones.ID_detalle ,detalle_factura_cupones.FK_ID_cupon,cupones.nombre_cupon,detalle_factura_cupones.cantidad FROM detalle_factura_cupones 
        INNER JOIN cupones ON detalle_factura_cupones.FK_ID_cupon = cupones.ID_cupon
        WHERE cupones.abreviatura_cupon = ? ";
    ...
    public function viewtotal(){
        $sql = "SELECT SUM(detalle_factura_cupones.sub_total) AS codo FROM detalle_factura_cupones
        WHERE detalle_factura_cupones.FK_ID_venta = (SELECT MAX(venta.ID_venta) FROM venta)";
        $params = array(null);
        return Database::getRows($sql,$params);
    }
    ...
    public function createcarrito(){
        $sql = "INSERT INTO detalle_factura_cupones(FK_id_cupon,cantidad,sub_total,FK_ID_cliente,correlativo,estadoventa) VALUES(?, ?, ?, ?, ?, 0)";
        $params = array($this->id_cupon, $this->cantidad, $this->subtotal, $this->id_usuario,$this->correlativo);
    ...
    public function validar_repetido(){
        $sql = "SELECT * FROM detalle_factura WHERE FK_id_cupon = ? AND FK_ID_cliente = ? AND estadoventa = 0";
        $params = array($this->id_cupon, $this->id_usuario);
        return Database::getRows($sql,$params);
    }
    public function validar_repetido2(){
        $sql = "SELECT cantidad FROM detalle_factura WHERE FK_id_cupon = ? AND FK_ID_cliente = ? AND estadoventa = 0";
        $params = array($this->id_cupon, $this->id_usuario);
    ...
    public function modificarrepetido(){
        $sql = "UPDATE detalle_factura SET cantidad = ?,sub_total = ? WHERE  FK_ID_cliente = ? AND FK_id_cupon = ? AND estadoventa = 0";
        $params = array($this->cantidad, $this->subtotal, $this->id_usuario, $this->id_cupon);
        return Database::executeRow($sql, $params);
    }
    ...
    public function validarCompra(){
        ...
        INNER JOIN producto ON detalle_factura.FK_id_cupon = producto.id_cupon
        INNER JOIN cliente ON venta.FK_ID_cliente = cliente.ID_cliente
        WHERE detalle_factura.estadoventa = 1 AND venta.FK_ID_cliente = ? AND detalle_factura.FK_id_cupon = ?
        ORDER BY venta.ID_venta ASC";
        $params = array($this->id_usuario,$this->id_cupon);
        return Database::getRows($sql, $params);
    }

    public function readCarrito(){
		$sql = "SELECT ID_detalle,FK_ID_venta, FK_id_cupon, cantidad,sub_total,FK_ID_cliente, estadoventa  FROM detalle_factura WHERE ID_detalle = ?";
		$params = array($this->id);
		$detalle = Database::getRow($sql, $params);
		if($detalle){
			$this->id = $detalle['ID_detalle'];
			$this->id_venta = $detalle['FK_ID_venta'];
			$this->id_cupon = $detalle['FK_ID_proucto'];
